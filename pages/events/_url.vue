  ...
  #event.bb( itemscope itemtype="http://schema.org/Event" v-if="!item.error"): .wrapper.pt4
    .row
      .col.col-xs-12.col-sm-4
        ...
        p.f3
          span.date( itemprop="startDate" :content="$moment(item.start_date).toISOString()" ) {{$moment(item.start_date).format('ddd D MMM')}}  
          span.time( itemprop="doorTime" ) {{ $moment(item.start_date).format('HH:mm') }}
          //- span.date( itemprop="endDate" :content="$moment(item.end_date).toISOString()" ) {{$moment(item.end_date).format('ddd D MMM')}}  
          //- span.time( itemprop="doorTime" ) {{ $moment(item.end_date).format('HH:mm') }}
        .tags.mt1
        ...
        .people( v-if="item.external_participants || item.internal_participants.length > 0" )
          ...
          .ext-participants 
            p(
              v-for="ext, i in item.external_participants"  
              itemprop="performer"
              itemscope itemtype="http://schema.org/Person" 
              :key="i")
              a( 
                itemprop="url"
                :href="linkify( ext.url )" 
                :title="ext.name" )
                span( itemprop="name") {{ext.name}}
          .int-participants 
            p(
              itemprop="organizer" 
              itemscope itemtype="http://schema.org/Person"
              v-for="int, i in item.internal_participants" 
              :key="i")
              nuxt-link( 
                :to="`/members/${int.directus_users_id.url}`"  
                content="performer") 
                span( itemprop="name" ) {{int.directus_users_id.first_name}} {{int.directus_users_id.last_name}}
        .locations( v-if=" item.location.length > 0" )
          h2.mtb1 Address
          .location( 
            v-for="l, i in item.location" 
            :key="i" 
            itemprop="location"  
            ...
            p
              a( 
              target="_blank" 
              v-bind:href="`https://maps.google.com/?q=${l.locations_id.map.lat},${l.locations_id.map.lng}`" ) 
                span( itemprop="name" ) {{l.locations_id.title}}
            p( 
              style="white-space: pre" 
              itemprop="address" 
              itemscope 
              itemtype="http://schema.org/PostalAddress" ) 
              span( itemprop="streetAddress") {{l.locations_id.address}}
        .links( v-if=" item.links " )
          h2.mtb1 Links
          .link( v-for=" l, i in item.links" :key="i"): a( 
            :href="l.url" 
            :title="l.title" 
            target="_blank") {{l.title}}
      .col.col-xs-12.col-sm-8
        dynamic-image.mt1( :file="item.cover" )
        .description.mt2.mb2.html( v-html="item.description" itemprop="description" )
